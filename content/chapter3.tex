...
The aim of this implementation is to showcase the practicability and performance of a redundant architecture that applies \abr{DCPS} concepts for finding a consensus in an \abr{ETCS} context, even in the presence of individual component failures.
\\

At first in this chapter, the structure of the implemented system is presented.
Therefore, the utilized hardware, the network topology, and the software structure is described.
In the second section, the way the system manages its state is explained.
Afterwards, the system's behaviour is outlined in two steps.
First, a leader election algorithm - the basis for further processing - is shown.
Second, the decision making algorithm, that is based on redundant computation and majority voting, is presented.
Finally in this chapter, methods for system recovery are described.
This includes an active hardware redundancy approach as well as a hardware watchdog.
...
\begin{figure}[!hb]
	...
	\caption{The on-board unit's program, that runs on each replica, consists of three major components. The \texttt{input} processing reads inputs and performs the voting. While the \texttt{state manager} components administers the system's global state, the \texttt{consensus} component ensures that a leader is present in the system and provides an interface to perform a redundant processing and voting.}
...
Information about \abrpl{MA}, linked balises, and balise telegrams can be transferred to the system via the input interface.
Via the result interface, the votet result for a corresponding input is presented.
\\
...
The system's state is stored as a global state in \abr{DDS} state topics and censequently managed by the middleware.
...
Further, the currenly valid \abr{MA} is transmitted to late joining replicas.
\\
...
One encodes the estimated position and two delimit an interval, the so called confidence interval, that certainly contains the train's true position.
...
However, before starting a new journey, it is necessarey to ensure that any information about previous \abrpl{MA} and linked balises is no longer accessible.
...
The basis for any processing is layed by the choice of a system leader.
...
Afterwards, an algorithm for creating a voted decision based on redundant results is presented.
For both algorithms, safety and timing considerations are described.
\\

In order for the application to notice that data samples are available on topics, \abr{DDS} \texttt{WaitSets} are used.
\texttt{WaitSets} are preferred over \abr{DDS} \texttt{Listeners} because they are state-based rather than event-based, which means they trigger as long as a certain state is present rather than when an event is triggered.
...
As a result, the middleware logic cannot be blocked by application logic.
\\
...
This is because topics in \abr{DDS} need to be in a certain data format.
...
Each replica manages its own term, its current role, and voting information as a private replica state.
\\
...
Each replica can only vote for a single other replica in every term.
...
Afterwards, it grants the vote for the promoting replica.
...
For each vote reply that is received by a promoting replica, it is first checked whether the replica has left the candidate state in the meantime, which makes the entire vote reply irrelevant.
...
\begin{figure}[!hb]
	...
	\caption{Replicas in \texttt{Raft} can be in one of three states. When a follower receives no heartbeat messages from a leader, it starts an election and tries to become the new leader. A replica remains in candidate state either until it became the new leader or until it receives a heartbeat from another leader in a higher term. Leaders operate until they fail or until they discover that their term is outdated. This diagram was taken from~\cite{RaftConsensusPaper}}
...
It is important for the system's safety that only one leader is present at any time.
...
This can be ensured by means of specialized multiplexing hardware that filters out simultaneous outputs.
Other hardware, for example gravity-controlled switches, can be used to ensure that a train stops when no replica is active at all.
\\
...
In case of a network partition, where one or multiple replicas are separated from the others, split brain situations with two leaders - one for each subnet - could happen.
However, the quorum required for voting prevents replicas in a partitioned network to become leader and thereby prevent split brain situations from leading to system failures.
...
\begin{figure}[!hb]
	...
	\caption{One part of the leader election process is the monitoring of heartbeat messages. This task is performed by replicas that are in follower state and is used to identify the absence of a leader. When no heartbeat messages are received for a certain period of time, a new leader election starts.}
...
An expiring \textit{leader ready timeout} indicates that it was impossible for the system to elect a new leader for the given term.
...
The problem that not enough replicas are active for an election can be solved by specifying a maximum number of consecutive terms that an election is allowed to fail.
...
Because a replica can become leader or follower at any point in time, it cannot be predicted when it transitions from publisher to subscriber or from subscriber to publisher.
This can be a safety hazard because registering an publisher or subscriber can fail and it takes time to register an \abr{DDS} entity to a topic.
Therefore, every replica is setup to simultaneously act as a publisher and subscriber to every topic, with an exception to the \texttt{Input} topic.

However, this also means that each replica receives its own messages.
Therefore, after reading each data sample, the application code must manually verify that the message was not published by the same replica that is reading the message.
...
A coarse overview about how the redundant computation and voting is performed is presented in~\autoref{fig:SeqCollectResults}.
\\
...
Third, the corresponding input is marked as processed on the \texttt{Input} topic by disposing the corresponding \abr{DDS} instance.
...
Communication between the leader and its follwers happens via \abr{DDS} topics.
A \texttt{WaitSet}, attached with a \texttt{ReadCondition}, is used to recognize new messages on the \texttt{Input} topic.
As soon as a new input message is recognized by the leader, it sends a \textit{process} request.
...
\item The balise telegram has been received at the position where the balise should be, according the the linking information.
...
Therefore, a timeout is attached to the \texttt{Input} topic's \texttt{WaitSet} that expires when no input message is received for a specific period of time.
...
In the following, both monitoring the braking curve and handling critical inputs is reffered to as \textit{input processing}.
\\

When the leader crashed during the small time frame between serving the voted decision as the system's output and marking the corresponding input as processed, an input could potentially be processed twice.
However, it is assumed to be very unlikely that such a situation happens.
In an alternative situation, when the leader would not mark the input as processed, it could happen that the leader crashes while processing the input and the input would not be processed at all.
...
In order for the replicas to generate deterministic and relevant decisions, every replica requires access to the system's most recent global state.
...
A possible way to reduce the chance of such situations, a processing where the leader's decision was outnumbered in the voting should be repeated.
However, only if the repetition would not jeopardise any time constraints.

\subsubsection{Timing Considerations}

After the leader send a decision request to every other replica and generated a decision itself, it waits for responses to continue with a voting.
Since the system consists of independent replicas, it is not possible to predict how long it will take the other replicas to generate a decision.
In order to prevent the voting process from being deferred indefinately, a timeout is attached to the \texttt{WaitSet} for the \texttt{AppendEntriesReply} topic.
...
In the first situation, a voting is possible without any problems.
...
In case an even number of decisions has been received, split votes in the voting process are possible and the most restrictive one has to be choosen.
...
It is not possible for the presented system to cope with such a situation because it would require a distributed timeout that is not possible for an asynchronous system~\cite{FLPProblemConsensus}.
...
If the replica has was assumed to be crashed does respond though, the spare can be deactivated again by publishing a corresponding message to the \texttt{ActivateSpare} topic.
\\

For creating a hot standby, a component must be initalized in spare mode.
This implies that the component is initialized as any other component and additionally subscribes to the \texttt{ActivateSpare} topic.
...
Upon being activated, a spare component participates in the input processing process by responding decisions to the system's leader.
\\
...
The \textit{Revolution Pi Connects} install a configurable hardware watchdog in form of a watchdog timer on a dedicated hardware chip.
The timer must be manually and periodically resetted.
When it is not resetted, the timer expires and the corresponding \abr{PLC} gets restarted.
This allows the watchdog to restart a replica when the on-board unit software stops responding.
\\

In order to take advantage of the hardware watchdog, the heartbeat mechanism that is used for the consensus algorithm is exploited.
Each time the leader sends a heartbeat, it also resets the watchdog timer.
For followers, the watchdog timer is reset upon receiving a heartbeat messages.
...
On the other hand, an appropriate watchdog timeout must be small enough to restart a faulty component as soon as possible but long enough to not trigger false positives.
...
This is analyzed in~\autoref{subsec:LeaderElectionEval} and forms the watchdog timer's lower limit.
