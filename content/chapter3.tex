...
The aim of this implementation is to showcase the practicability and performance of a redundant architecture that applies \abr{DCPS} concepts for finding a consensus.
...
%\todo{No active hardware redundancy??}
...
The system's state is stored in \abr{DDS} state topics and thereby managed by the middleware.
...
\SetKwData{Term}{term}
\SetKwData{Sender}{senderID}
\SetKwInOut{Input}{input}
\SetKwInOut{Output}{output}

\Input{A vote request with a \textit{\Term} and a \textit{senderID}}
\Output{true if the vote for \textit{senderID} in \textit{\Term} was granted, false otherwise}
\BlankLine
...
\SetKwData{VoteTerm}{voteTerm}
\SetKwData{VoteGranted}{voteGranted}
\SetKwInOut{Input}{input}
\SetKwInOut{Output}{output}

\Input{A vote reply with a \textit{\VoteTerm} and a \textit{\VoteGranted} flag}
...
Increase number of granted votes in the election term\;
	\lIf{got enough votes}{become leader
							return\;}
}
\end{algorithm}
...
On the other hand, the cases of network delays or too few replicas in the system represent a safety risk and should be dealt with accordingly.

\todo{There is also some more processing time}
...
\todo{Further explain how message is marked as processed}
