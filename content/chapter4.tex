...
This value is not far from the 100~Mbit/s supported by the \textit{Revolution Pi's} hardware.
\\
...
\begin{figure}[!t]
	...
	\caption{Messages that were published to state topics over a period of three scenarios that start at \textbf{S1}, \textbf{S2}, and \textbf{S3}. For the first and third scenario, three balises were linked. The second balise includes an unlinked balise and therefore, only two are linked. Further, a crashed leader (at point \textbf{LC}) does not affect the system state's up-to-dateness since the same amount of messages are published to the state as before \textbf{LC}.}
...
While the train is driving, it is the leader's responsibility to update the system's global state.
Hence, most messages are sent by the system's leader, ten alone due to heartbeat messages.
This explains the prominent increase in sent messages during operation.
The remaining messages accrue from a system-wide braking curve monitoring, for which requests and responses are transmitted.
...
This circumstance is depicted in~\autoref{fig:PlotConsensusMessagesSent}.
...
\begin{itemize}
\item \textit{DDS\_DomainParticipant\_create\_(publisher|subscriber|topic)} Used for creating \abr{DDS} publishers, subscribers or topics.
\item \textit{DDS\_DomainParticipant\_delete\_(publisher|subscriber|topic)} Used for deleting \abr{DDS} publishers, subscribers or topics.
...
Further, \texttt{WaitSets} and \abr{QOS}-settings can be used to declare time restrictions and requirements.
This facilitates reliable and predictable machine-to-machine communication.

In addition, the middleware supports the automatic discovery of replicas so that the number of used replicas is dynamically scalable.
...
However, this can lead to a sudden allocation and deallocation of dynamic memory, resulting in unpredictable timings.
