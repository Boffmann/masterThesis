\chapter{System Demands and Comparisons}
\label{chptr:redundantSystemsCompare}
Due to the high demands on the safety and reliability of an on-board unit, it is to be developed as a redundant system.
As previously described, there are different approaches to introduce redundancy.
...
Furthermore, possible security risks and catastrophic failures must be known in order to be able to wight up the safety of the architectures.
\\

In the first section of this chapter, an on-board system's demands are described.
This will be done based on a subset of the \abr{ETCS} specification.
Afterwards, building blocks for defining communication in \abr{DCPS} systems with \abr{DDS} are presented.
Finally, a redundant architecture is established and evaluated based on the system's demands and its realizability with \abr{DDS}.
...
The system should be able to receive a \abr{MA} that contains a permission to drive.
Therefore, a \abr{MA} consists of a start- and an end position that limit an area where the train is authorized to move.
...
The phase in which the balise positions and identifiers are received is called \textit{linking phase}.
Further, the tuple of balise position and identifier that are trasmitted during the linking phase are called \textit{linked balises}.
...
During this process, situations may arise that affect the train's safety.
The following problems should be able to be detected by the system and lead to the train being stopped:
...
The safety requirements for the system are based on the fault classes \textbf{F1}-\textbf{F4} from~\autoref{sec:techniquesSafetyReliabilit}\cite{CristianFaultModel}.
Consequently, byzantine faults are not covered.
Further, the communication channel as well as the \abr{DDS} middleware are expected to be safe and reliable.
Hence, no data is lost or corrupted while being transmitted.
...
\begin{itemize}
\item \textbf{Reliability:} How likely does the system still operate as specified in case of faults?
...
In order to evaluate the redundant system's reliability, it is mathematically evaluated using Marcov Chains, as described in~\autoref{sec:safetyEvaluation}.
To assess the system's safety, situations where faults can lead to catastrophic failures are assessed for each architecture. 
...
The minimal communication overhead required in \abr{TMP} is between the replicas and the voter.
...
\item \ChallengeWR When more than one replica produces a wrong result, the system's safety is affected.
...
Although \abr{TMR} with spares improves the replicated system's reliability by allowing more replicas to fail (\ChallengeWR), the voter remains a single point of failure (\ChallengeVoter) and the system's throughput has not changed (\ChallengeThrough).
