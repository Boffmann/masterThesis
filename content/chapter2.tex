...
There are other ways for ensuring deadlines in \abr{DDS}, such as using \texttt{WaitSets} with timeouts.
...
\todo{Check that gls abrevates this correctly}
...
In order to overcome \ChallengeWR, each replica's internal consistency needs to be monitored.
...
Further, the spare replica's state needs to be synchronized with the other replicas to make pertinent decisions, which can be assured by managing a global system state thorugh \abr{DDS}.
\\
