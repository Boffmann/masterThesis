...
\section{European Train Control System}
...
A \abr{MA} not only encodes the train's permission to move, it also encodes track information, such as slopes or speed limitations, as well as information about other trains on track.
Any \abr{MA} is provided by external entities, for example via track-side components or via radio frequencies, and monitored by a on-train system.
...
\abrpl{MA} as well as route data and the maximum allowed speed are transmitted through fixed eurobalises.
Parallel to that, a breaking curve is permanently calculated.

For \abr{ETCS} level two, signalling and \abr{MA} data is transmitted via radio frequencies and displayed to the driver.
While eurobalises are only used for aligning the calculated train position with its actual position, the train is constantly communicating its position, its speed, and its direction to a central interlocking.

In the final \abr{ETCS} level three, any track-side component is abolished and the entire communication takes place via radio signals.
...
A \texttt{WaitSet} can be attached with different \texttt{Conditions} and allows the application to wait until one or more of the attached conditions are satisfied, or until it times out.
...
Both types of voters have in common, that they require some kind of time constraint synchronization with the replicas to collect their intermediate results, perform the voting, and recognize failed or delayed redundant results.
...
In component-checking, a program is used to monitor and validate a component's hardware, for example its memory, its clock, or its \gls*{CPU}.
...
Since Song \etal exerted a topology which is similar to the one used in this work - namely a network switch with a 100Mpbs Ethernet link for communication and external nodes - their findings can be seen as a affirmation for the approach used in this work.
...
\paragraph{Time and availability for consensus algorithms.}
...
\todo{Bring into relation to my work}