...
In order to facilitate interoperability between national railway systems throughout Europe, the signaling and speed control system has been standardized by the European Union under the term \abr{ERTMS}.
\abr{ETCS} defines the signaling and train control component of \abr{ERTMS}~\cite{ETCS26}.
As such, it is responsible for monitoring a train's speed, calculating the permissible speed restrictions, and coordinating the collaboration between track-side components, such as \abr{ETCS} euro-balises, and on-board systems.
...
\begin{definition}
\todo{Better: Does not allow faults to lead to catastropic failures}
