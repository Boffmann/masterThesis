...
Thus, the replicas need to communicate with each other and synchronize their results to act as a single unit in their environment.
...
A system that is located on the operating train - called on-board unit or \abr{EVC} - monitors compliance with the current \abr{MA}.
The track-side components, as well as the on-board unit, are standardized in \abr{ETCS}.
...
Hence, a train operating under \abr{ETCS} does not know its exact position but can only locate itself within a certain range.
...
Thus, when a train crosses a linked balise and receives the balise telegram, the on-board unit can reset the train's confidence interval to the balise's position.
...
\abr{ETCS} level three is not fully standardized yet but is intended to replace track-vacancy detection with on-board technology.
...
Further, their work provides an overview of Vortex OpenSplice's \gls*{QOS} policies and features to ensure reliability, availability, data delivery, and resource usage.
...
At state \textbf{(1)}, assuming that the system is homogeneous redundant, the probability that one of the replicas fails is $3\lambda$.
In state \textbf{(2)}, the system applies failure detection and mitigation procedures and has a chance of $F(t)$ for successful reconfiguration, which allows the system to continue its work.
...
In state \textbf{(4)}, there is again a $2\lambda$ change for one of the remaining components to fail.
This would not result in a system failure, though, because failure detection allows the voter to distinguish a good result from a bad one.
...
After the system produces an output based on its input information, the output is also encoded and afterward compared to the encoded input.
...
Typically, as Bemment \etal pointed out, an in-depth redundancy evaluation towards safety and reliability can only be made for well-defined use cases where hazards, risks and potential accidents are known.
...
Since Song \etal exerted a topology that is similar to the one used in this work - namely a network switch with a 100~Mbit/s Ethernet link for communication and external nodes - their findings can be seen as an affirmation for the approach used in this work.
