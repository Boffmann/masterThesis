...
At the same time, \abr{SIL} 4 certified \abr{ATP} systems with a small \abr{PFD}~\cite{SallekSIL} are possible.
Further, the standardization of signaling and control components in the \abr{ETCS} is intended to facilitate train operation across borders and increases the significance of \abr{ATP} systems~\cite{YIN2017RNDofATO}.
...
This thesis investigates whether the \abr{DDS} standard is applicable for building redundant, distributed, real-time, and safety-critical systems in the \abr{ETCS} domain.
...
A selection of related work that uses \abr{DDS} in safety-critical applications is presented and set in relation to this work at the end of chapter 2.
\\
...
The system is comprised of four homogeneous and interconnected \textit{Revolution Pi} from Kunbus~\cite{Kunbus} and utilizes \textit{OpenSplice DDS} from ADLINK~\cite{VortexOpenSplice} as a \abr{DDS} framework.
A \textit{Revolution Pi} is a robust \abr{PLC} that builds upon a \textit{Raspberry Pi} and features a real-time capable \abr{OS}.
